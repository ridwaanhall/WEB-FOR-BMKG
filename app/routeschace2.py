...
@app.route("/cuaca-diyogyakarta/<area_id>/<parameter_id>/<timerange_id>")
def cuaca_diyogyakarta_area_id_parameter_id_timerange_id(area_id, parameter_id, timerange_id):
    url = 'https://data.bmkg.go.id/DataMKG/MEWS/DigitalForecast/DigitalForecast-DIYogyakarta.xml'
                ...
                for parameter in parameters:
                    if parameter['@id'] == parameter_id:
                        ...
                        if isinstance(timeranges, list):
                            for timerange in timeranges:
                                if timerange['@id'] == timerange_id:
                                    selected_timerange = timerange
                                    break
                        else:
                            if timeranges['@id'] == timerange_id:
    ...
    if selected_area and selected_parameter and selected_timerange:
        ...
        # Update timerange_id based on the type of timerange
        timerange_type = selected_timerange.get('@type')
        if timerange_type == 'daily':
            timerange_id = selected_timerange.get('@day')
        elif timerange_type == 'hour':
            timerange_id = selected_timerange.get('@h')
        ...
        return render_template(
            ...
            timerange_description=selected_timerange['@description'],
            unit_values=unit_values
        )
